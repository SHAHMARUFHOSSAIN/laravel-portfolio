...
<head>
    ...
    <meta name="description" content="{{ $profile?->name ?? 'Portfolio' }} - {{ $profile?->title ?? 'Full Stack Developer' }}">
    <title>@yield('title', $profile?->name ?? 'Portfolio' . ' | ' . $profile?->title ?? 'Full Stack Developer')</title>
